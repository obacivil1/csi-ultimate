...
async function extractWithCorrectSelectors(name, listingUrl, waitMs, getAdLinks, siteConfig) {
  console.log("\n" + "=".repeat(70))
  console.log("  " + name)
  ...
  console.log("  Ad links: " + adLinks.length)
  ...
  adLinks.slice(0, 8).forEach((url, i) => {
  ...
  console.log("\n  === RESULTS: " + successCount + "/5 extracted ===")
  await browser.close()
}

// ═══ OLX.COM.PK with correct selectors (from ad page inspection) ═══
await extractWithCorrectSelectors(
  "OLX.com.pk (correct selectors)",
  ...
  },
  {
    ...
    selectors: {
      ...
      phone: ["a[href^='tel:']"],
      email: ["a[href^='mailto:']"],
      location: ["span._8206696c"],
      price: ["span._24469da7"],
    },
    extraction: {
      phoneRegion: "PK",
      countryCodes: [92],
...
// ═══ EXPATRIATES.COM with verified selectors ═══
await extractWithCorrectSelectors(
  "Expatriates.com (correct selectors)",
  ...
  async (page) => {
    ...
    return await page.evaluate(() => {
      const seen = new Set()
      return Array.from(document.querySelectorAll("a[href*='/cls/'][href$='.html']"))
        .map(a => a.href)
